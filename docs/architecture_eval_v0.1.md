...
- The bridge is *known* but recorded only inside mmd-authoring materials: `skills/architecture/mmd-diagram-authoring/references/project-diagram-layers.md` records six diagrams, of which 4 (*conversion job lifecycle*: render source → extract → build IR → map objects → generate PPTX) and 5 (*validation evidence lifecycle*) are precisely the missing joints between the lifecycle layer and the pipeline — and they do not exist yet.
...
- Partially documented (traces only): artifact sessions and the revision loop appear in diagrams 01–02 ("Artifact revision session", "Run revision workflow") and in the mmd-authoring layer map's planned layer 6 — "create revision request bundle, **run authenticated revision**, register or publish revised source artifact" (`skills/architecture/mmd-diagram-authoring/references/project-diagram-layers.md`).
